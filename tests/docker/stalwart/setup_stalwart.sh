...
for i in $(seq 1 $max_attempts); do
    # The JMAP session endpoint redirects once the database is initialised.
    if curl -s -o /dev/null -w "%{http_code}" "http://localhost:${HOST_PORT}/.well-known/jmap" 2>/dev/null | grep -q "307"; then
...
max_jmap_attempts=15
for i in $(seq 1 $max_jmap_attempts); do
    RESPONSE=$(curl -s -u "${TEST_USER}@${DOMAIN}:${TEST_PASSWORD}" -L \
        "http://localhost:${HOST_PORT}/.well-known/jmap" 2>/dev/null)
    if echo "$RESPONSE" | grep -q "urn:ietf:params:jmap:calendars"; then
...
echo "  JMAP session URL: http://localhost:${HOST_PORT}/.well-known/jmap"
